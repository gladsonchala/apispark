        ...
        if filename.endswith(".py") and filename != "__init__.py":
            ...
            for name, function in inspect.getmembers(module, inspect.isfunction):
                ...
                if name.startswith("get_"):
                    ...
                    router.get(route_path)(function)
                elif name.startswith("post_"):
                    route_path = f"/{module_name}/{name[5:]}"
                    router.post(route_path)(function)
                elif name.startswith("put_"):
                    route_path = f"/{module_name}/{name[4:]}"
                    router.put(route_path)(function)
                elif name.startswith("delete_"):
                    route_path = f"/{module_name}/{name[7:]}"
                    router.delete(route_path)(function)
                # Add more HTTP methods as needed

# Automatically call the route registration when the module is loaded
register_routes()
